.. meta::
    :description: Comprehensive guide to the NKI programming model for AWS Neuron SDK, covering memory hierarchy, tensor operations, tile-based programming, and SPMD execution model.
    :keywords: NKI, AWS Neuron, Programming Model, SPMD, Tensor Operations
...
.. note:: 
   This programming model documentation covers NKI Beta 1 and earlier.
   For the latest NKI programming model, see :doc:`NKI Language Guide (Beta 2) </nki/deep-dives/nki-language-guide>` and breakout topics in the other sections of the NKI docs.
...
:ref:`NeuronDevice Architecture Guide <nki_arch_guides>`.
...
model. See :ref:`NeuronDevice Architecture Guide <nki_arch_guides>` for the exact values.
...
:doc:`nki.isa.dma_copy </nki/api/generated/nki.isa.dma_copy>`, and
computed output tensors of the kernel must be stored back into the HBM from SBUF
using the same API ``nki.isa.dma_copy`` before the host can
access them.
...
:doc:`nki.language.ndarray </nki/api/generated/nki.language.ndarray>` and
:doc:`nki.language.zeros </nki/api/generated/nki.language.zeros>`.
...
such as loop variables, :doc:`nki.language.program_id </nki/api/generated/nki.language.program_id>` and
:doc:`nki.language.affine_range </nki/api/generated/nki.language.affine_range>`.
...
data layout and tile size constraints.
...
the :doc:`nki.isa.nc_matmul </nki/api/generated/nki.isa.nc_matmul>` API,
...
:doc:`nki.isa.nc_matmul </nki/api/generated/nki.isa.nc_matmul>` that can take ``[M, K]`` and ``[K, N]``
...
Engines. See :doc:`nki.isa.tensor_reduce </nki/api/generated/nki.isa.tensor_reduce>` API as an example.
...
.. nki_example:: /nki/examples/layout-pass.py
   :language: python
   :linenos:
   :whole-file:
...
.. nki_example:: /nki/examples/layout-violation.py
   :language: python
   :linenos:
...
.. nki_example:: /nki/examples/layout-loop.py
   :language: python
   :linenos:
...
``[0, 1]``. :doc:`nl.affine_range </nki/api/generated/nki.language.affine_range>`
...
(see :doc:`Matmul Tutorial </nki/tutorials/matrix_multiplication>` for an example).
Otherwise, :doc:`nl.sequential_range </nki/api/generated/nki.language.sequential_range>`
...
.. nki_example:: /nki/examples/layout-dynamic-loop.py
   :language: python
   :linenos:
...
efficiently stride the SBUF/PSUM memories at high performance for all NKI APIs that access on-chip memories.
...
2. Transpose between two free-dimension axes, which is achieved
   via a :doc:`nki.isa.dma_copy </nki/api/generated/nki.isa.dma_copy>` API,
...
:doc:`nki.language.program_id </nki/api/generated/nki.language.program_id>`
...
:doc:`nki.isa.nc_matmul </nki/api/generated/nki.isa.nc_matmul>` API,
...
we suggest transposing input ``A`` and invoking :download:`another NKI kernel </nki/examples/mm-nisa-spmd.py>` instead,
which solely performs matmul operations on Tensor Engine
using :doc:`nki.isa.nc_matmul </nki/api/generated/nki.isa.nc_matmul>` without extra overhead in changing
input layouts to meet :ref:`LC#1 <nki-pm-layout>`.

.. nki_example:: /nki/examples/mm-nl-spmd.py
   :language: python
   :linenos:
   :whole-file:
