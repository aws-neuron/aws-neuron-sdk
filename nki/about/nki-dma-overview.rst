...
Before reading this doc, it may be helpful to refer to :doc:`Introduction to memory hierarchy in NKI </nki/about/memory-hierarchy-overview>`.
...
The points on the graph refer to various Free (Dimension) Byte values (that is, bytes per partition). We see that at 4096 free bytes, we are able to nearly saturate DMA bandwidth. In the DMA Deep Dive, we will review a techniques for reshaping/folding input tensors to increasing free dimension read sizes.
