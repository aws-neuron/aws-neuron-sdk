...
Starting from *Neuron 1.16.0*, Neuron framework extensions (TensorFlow Neuron, PyTorch Neuron or MXNet Neuron) will be packaged together with 
...
Starting from *Neuron 1.16.0*, TensorFlow Neuron model server will be packaged together with ``libnrt.so`` and will expect ``aws-neuron-dkms`` 
